...
**Member**
- **Home tab** (default landing page) — themed identically to the rest of the app (no bespoke colours): an oversized-type hero banner, a warning banner (with a direct "Return this book" link) whenever you have an overdue borrow or an unpaid fine, a **Reading Goals** card, and six collapsible sections (My Borrowed Books, My Fines, Past Borrows, My Reservations, My Wishlist, and a "From the collection" 6-book preview); only one section is expanded at a time (click its heading to open it) and the rest collapse to just their header bar. Sections render as a plain divider list (no card/box chrome) with headings sized to match the rest of the app, not a bespoke oversized style. My Fines renders as book cards (cover, title/author, paid/unpaid badge, due date, fine amount) rather than a table, matching the other "my stuff" sections
- **Reading Goals** — set a personal target (weekly, monthly, or yearly number of books) from the Home tab; a progress bar tracks books marked "complete" against the current period, plus a running "books read this year" count regardless of the goal's own period. Marking a book complete happens as a checkbox (checked by default) in the Return modal — separate from the return itself, so a book borrowed and returned unread doesn't count toward the goal
